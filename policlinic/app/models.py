...
class User(AbstractUser):
    middle_name = models.CharField(
        max_length=30, default=None, blank=True, null=True, verbose_name="Отчество"
    )
    policy = models.OneToOneField(
        Policy, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Полис"
    )
...
class Doctors(models.Model):
    ...
    specialization = ForeignKey(
        Specialization, on_delete=SET_NULL, null=True, verbose_name="Специализация"
    )
...
class Institution(models.Model):
    name_institution = models.CharField(
        max_length=255, verbose_name="Название института"
    )
...
class ResearchResults(models.Model):
    user = models.ForeignKey(
        User, on_delete=CASCADE, default=None, verbose_name="Пользователь"
    )
    name_research = models.CharField(
    ...
    file_path = models.OneToOneField(
        FilePDF, on_delete=CASCADE, verbose_name="Путь к файлу"
    )
    institution = models.OneToOneField(
        Institution,
        on_delete=models.SET_NULL,
...
class Records(models.Model):
    ...
    doctor = models.ForeignKey(
        Doctors, on_delete=models.SET_NULL, null=True, verbose_name="Доктор"
    )
